...
我们已经提供了一些实用脚本，它们可以辅助开发或者包含一些进程中有用的标准操作。
...
它为记录和发布 kdb+ 进程中的函数/变量/表或查看定义提供了一种机制。它提供了通
过名称和定义来搜索函数的功能。除此之外还提供了一个函数来按降序返回当前进程中
每个变量的大致内存使用量。

可以通过 .api.add 函数来添加定义。变量可以标记为公有的还是私有的，并给出描述、
参数列表以及返回类型。搜索函数将找出并返回所有匹配的值，不管这些值是否经过了
预先定义。

Whether a value is public or private is defined in the definitions
table.
变量是公有还是私有会定义在一个表格中。 
If not found then by default all values are private, except those
which live in the .q or top level namespace.
如果没有定义的话，那么除了存在于.q或顶级命名空间中的值都会默认被认为是私有的。

.api.f is used to find a function, variable, table or view based on a
case-insensitive pattern search.
.api.f可以被用于搜索函数,变量,表格或视图,它本身对大小写是不敏感的.
If a symbol parameter is supplied, a
wildcard search of \*\[suppliedvalue\]\* is done. 
当给到一个symbol作为参数,函数将会是采用\*\[suppliedvalue\]\*通配符的方式进行查询.
If a string is
supplied, the value is used as is, meaning other non-wildcard regex
pattern matching can be done.
当给到的是一个string参数,那么函数将会采取非通配符的形式进行搜索.
...
namespaces.
使用.api.torqnamespaces可以返回torq命名空间的symbol list.
...
inputted namespace.
.api.exportconfig使用.api.f`返回的表格来给出输入的命名空间下的变量的当前值和描述.
This can be used to quickly see what configurable
variables are currently set to.
这个方法可以用来快速查看当前设置的配置变量的值.
...
.api.exportallconfig返回的是所有可获取的命名空间下的变量和值,返回的结构和
.api.exportconfig是一一致的.
...
.api.p和.api.f类似,但只会返回公共函数. .apip.u的话只会包括用户定义的变量,它排除
了q系统的私有变量和.q, .Q, .h 以及 .o命名空间下的变量. 
 
.api.find is a more general version of .api.f which can be used to do
case sensitive searches.
.api.find可以看做一个.api.f通用版本,能被用于进行大小写敏感的查询.

.api.s is used to search function definitions for specific values.
.api.s是被用来查询那些函数定义中的特殊值的.
...
views in the process, retrieved using -22!.
.api.m被用于查询进程中变量和视图的大概内存使用, retrieved using -22!.
Views will be re-evaluated
if required. Use .api.mem\[0b\] if you do not want to evaluate and
return views.
视图的大小将会重新计算.如果你不想重新计算和返回视图,可以使用.api.mem\[0b\].
...
.api.whereami\[lambda\]可以通过给定函数定义来检索函数名称,这在调试中很有用.
...
在common和handler文件夹中的这个文件都是被用来向api.q中添加接口用法的.
...
frequency specified by -t.
kdb+提供了一个单独的定时函数.z.ts,他是通过指定-t的值来周期性的触发.
We have provided an extension to allow
multiple functions to be added to the timer and fired when required.
我们提供了一个扩展来实现将多个函数添加到定时器中并在需要的时候进行解除.
The
...
are required.
实现的基本方式是通过把定时函数注册到一个表格中去,然后使用.z.ts周期性的去检查这个
表格并判断是否需要执行.
This is not a suitable mechanism where very high frequency
timers are required (e.g. sub 500ms).
这对于非常高频的机制来说是不太适合的.(e.g. sub 500ms)

There are two ways a function can be added to a timer- either as a
repeating timer, or to fire at a specific time.
我们提供了两种方式实现了将一个定时函数加到定时器中定时运行或在一个特定的时点进行解除.
When a repeating timer
is specified, there are three options as to how the timer can be
rescheduled. 
当指定一个定时器时,这有3种选项来指定定时器是如何周期运行的.
Assuming that a timer function with period P is scheduled
to fire at time T0, actually fires at time T1 and finishes at time T2,
then
假定一个定时函数间隔P周期进行运行,第一次是设置在T0运行,实际运行的时候是在T1执行的并
在T2时间结束.
...
-   mode 0 程序的下次运行将会在 T0+P;

-   mode 1 程序的下次运行将会在 T1+P;

-   mode 2 程序的下次运行将会在 T2+P.
...
当T2时间大于下一周期运行的开始时间时,模式0和1都存在潜在的风险导致计时器出错.
使用.api.p“.timer.\*”查看更多细节.
...
asynchronous calls. 
kdb+的进程之间能通过同步或者异步调用的方式进行交互.
Synchronous calls expect a response and so the
server must process the request when it is received to generate the
result and return it to the waiting client.
同步调用会收到一个返回结果,所以服务端必须一收到请求就开始处理并生成一个结果
返回给正在等待的客户端.
Asynchronous calls do not
expect a response so allow for greater flexibility.
异步调用不用返回结果,所以使用起来更灵活.
The effect of
synchronous calls can be replicated with asynchronous calls in one of
two ways (further details in section gateway):
同步调用的效果能通过以下两种方式的进行异步调用进行复制(更多细节参见geteway部分).
...
-   递延的同步调用:客户端发送异步请求,并且handle形成阻塞等待结果.这种方式允许
服务端更灵活的选择是么时候以及怎样处理查询请求.
...
- 异步回调:客户端发送一个封装好回调函数的异步请求,该函数会在结果生成后进行调用.
这种方式允许服务端灵活的选择什么时候以及怎样处理查询请求,并允许客户端在收到服务
端生成结果时继续进行处理.

The code for both of these can get a little tricky, largely due to the
amount of error trapping required.
这两种方式是的代码都有点复杂,很大程度上是因为需要捕捉大量的错误.
We have provided two functions to
allow these methods to be used more easily.
我们提供了两个函数以便与更简单的使用和这两种方式.
.async.deferred takes a list
of handles and a query, and will return a two item list of
(success;results).
.async.deferred接收一个handles list和一个查询请求,返回一个具有(success;results)
两项的列表.

    q).async.deferred[3 5;({system"sleep 1";syst em"p"};())]                                                                                                                                                                                     
    1    1   
    9995 9996
...
client when ready.
.async.postback接收一个handles list,一个查询请求和一个回调函数的名称或lambda函数.
请求会收到是否成功,结果会在处理好之后返回给客户端.
...
returning them from the cache if they are available and non stale.
cache.q提供了一种储存函数结果到缓存中并在缓存可用且未过期时使用他们进行返回.
 This
can greatly boost performance for frequently run queries.
这种机制极大的提升了那些经常使用查询的性能.
...
-   the result set is not too large and/or is relatively expensive to
      ...
      extracts.
      结果集不大或重新生成的成本过高.例如,缓存原始数据集就没有什么必要.

The cache has a maximum size and a minimum size for any individual
result set, both of which are defined in the configuration file.
每个单独的缓存都会有一个最大和最小的容量,这两个值会在配置文件中给定.
Size
...
大小的计算都是通过 -22! 实现的,这个方式只能给出一个大约(低估)的结果集大小.
在最极端的情况下可能只会得出真实大小的一半.
...
cache.
每当新的结果生成时都会进行检查,如果结果大小不超过最大的独立cache的size,那么
他就会被放入缓存中.
 If the new cache size would exceed the maximum allowed space,
other result sets are evicted from the cache.
如果新的缓存size超过了最大允许的尺寸,其他的结果集就将被清除.
 The current eviction
policy is to remove the least recently accessed result sets until the
required space is freed.
当前的清理原则是按照时间先后清理最为久远的数据直到空间足够.
The cache performance is tracked in a table.
Cache adds, hits, fails, reruns and evictions are monitored.
缓存的表现会被记录,缓存的新增,命中,失败,返回和清理都会被记录.

The main function to use the cache is .cache.execute\[function;
staletime\].
缓存相关最主要的函数是.cache.excute\[function;staletime\].
If the function has been executed within the last
staletime, then the result is returned from the cache. Otherwise the
function is executed and placed in the cache.
在过期时间内,这个函数执行的结果都会从缓存返回。换句话说,这个函数被执行并加入了缓存.
...
result returned:
如果自上次执行以来的时间已经超过了过期时间,函数将会被重新执行,缓存会被更新并进行结果返回.
...
an SMTP server.
这个库文件被提供来允许TorQ进程使用SMTP服务来发送邮件.
This is a wrapper around the standard libcurl library.
它是基于标准的libcurl库进行封装的.
The library file is currently available for Windows (32 bit), Linux (32
and 64 bit) and OSX (32 and 64 bit).
这个库当前允许在 Windows (32 bit), Linux (32 and 64 bit) and OSX (32 and 64 bit)下进行使用.
 The associated q script contains
two main methods for creating a connection and sending emails.
相关的q脚本中包括了两个方法分别来建立连接和发送邮件.
 The email
library requires a modification to the path to find the required libs -
see the top of email.q for details.
这个库需要修改修改一下(环境)路径来查找依赖库.查看email.q的顶部内容了解更多细节.
...
.email.connect接收一个字典参数并返回0i代表成功,或-1i代表失败.
...
邮件发送函数.email.send接收一个包含了邮件内容的字典作为参数.在发送邮件之前,必须先建立
邮件连接.发送函数会返回一个邮件长度的实数代表成功,或返回-1i代表失败.
...
注意,如果邮件发送得不频繁,那么邮件库必须要重新建立邮件连接才能发送
(这在初次连接之后会自动进行).
 In some circumstances it may be better to batch
...
在某些情况下,批量进行邮件发送或将邮件转移到其他进行进程再与SMTP服务器进行交互会更快一些.
...
我们还提供了两个其他的函数,  .email.connectdefault and .email.senddefault.
这个两个方法和上面的一样,只是说使用了配置文件中定义好的参数进行了传递.另外
.email.senddefault会自动进行邮件连接.
...
.email.test函数会尝试对默认配置的邮件服务进行连接,并像给定的邮件地址发送测试邮件.
需要将debug设置为2i (verbose)来追踪完整信息.

    q).email.debug:2i
    q).email.test `$"test@aquaq.co.uk"
    ...

A further function .email.sendviaservice can be used to send an email using 
the default mail server on a separate specified process and can be used to 
allow latency sensitive processes to offload this piece of functionality.
.email.sendviaservice函数允许通过一个额外指定的进程上的默认邮件服务发送对
延时敏感度高的邮件.


The function takes two parameters a process and a dictionary which should 
follow  the same format as .email.send.
函数接收进程和邮件内容字典(与.email.send一致)两个参数.
 The function uses the 
.async.postback Utility to send the email by calling .email.servicesend 
on the specified process. The postback function immediately returns a 
success boolean indicating that the the async request has been sent and 
when the function has been run on the server the results are posted back 
to the client function  email.servicecallback which logs the email status.
函数使用了.async.postback功能来调用了指定进程上的.email.servicesend.请求已发送就
会立即返回一个success boolean显示异步请求已经被发送,并在服务端的函数执行完后将结果
返回给客户端的email.servicecallback回调函数进行邮件状态的日志记录.
...
使用.api.p.email.\*查看更多邮件库额外的功能和细节.
...
如果你希望通过一个windows下的认证账户(e.g. Hotmail, Gmail)发送邮件,那么你需要
进行几步额外的操作来设置 usessl 为 True,并且Windows并不总能找到正确的证书位置.
步骤如下:
...
基于code.kx的一个轻量级的定制化的timezone转化函数库版本.它会从$KDBCONFIG文件夹中
加载时区信息表格.使用.api.p.tz.\*查看更多信息.
